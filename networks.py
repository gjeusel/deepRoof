...
class LargeNetSmallPictures(nn.Module):
    def __init__(self, input_shape):
        super(LargeNetSmallPictures, self).__init__()

        self.layer1 = nn.Sequential(
            nn.Conv2d(3, 64, kernel_size=3, padding=1),
            nn.ReLU(),
            nn.Conv2d(64, 110, kernel_size=3, padding=1),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2))

        self.layer2 = nn.Sequential(
            nn.Conv2d(110, 84, kernel_size=3, padding=1),
            nn.ReLU(),
            # nn.BatchNorm2d(32),
            nn.Conv2d(84, 84, kernel_size=3, padding=1),
            nn.ReLU(),
            nn.MaxPool2d(2))

        self.layer3 = nn.Sequential(
            nn.Conv2d(84, 64, kernel_size=3, padding=1),
            nn.ReLU(),
